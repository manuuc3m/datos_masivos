...
def refine_nieve(nievedata):
    ...
    print('values', values)
    mapping_empties = [ ' - ' if e == '' else f"{e} m" for e in values]
    # Necesario que sean siempre 7 valores
    if (len(mapping_empties) < 7):
        diff = [' - '] * ( 7 - len(mapping_empties) )
...
def refine_viento(vientodata):
    ...
    if (len(info) < 7):
        diff = [{'direccion': ' - ', 'velocidad': ' - '}] * ( 7 - len(info) )
        return info + diff
    return info